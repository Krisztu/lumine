...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const initData = async () => {
      ...
      if (isMounted) {
        ...
        await Promise.allSettled(promises) // allSettled - ha egy hiba van, a tobbi meg megy
      }
    }
    initData()
  ...
  const loadGrades = async (userData: any) => {
    try {
      ...
      if (userData?.role === 'student' || userData?.role === 'dj') {
        ...
        // admin mindent lat, url param nelkul
      } else {
        return
      }
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (roleToCheck === 'teacher' || roleToCheck === 'homeroom_teacher') {
          ...
          // admin -> ne irjuk felul, csak ha ures a lista
  ...
  const handleLogout = async () => {
    try {
      // kikapcsoljuk az auto redirectet
      setLoading(true)
      await logout()
      // kenyszeritett redirect
            ...
            <TabsContent value="schedule" className="space-y-3 sm:space-y-6">
              <ScheduleTab
                ...
                openAttendanceModal={(lesson) => {
                  // ha mar van jelenlét, ne nyissuk meg
